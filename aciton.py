...
# training settings
TOTAL_EPOCHS = 120  
LEARNING_RATE_STEP = [0.1, 0.05, 0.01, 0.005] 
LEARNING_RATE_BOUNDARY = [30, 60, 90] 
#TOTAL_EPOCHS = 60 
            ...
            for ep in range(TOTAL_EPOCHS): 
                ...
                for itr in range(0, total_iteration):   
                    ...
                    if np.mod(itr, PRINT_FREQ) == 0:
                        ...
                        # print weight image   
                        avn1 = model.get_avn(1, img_feat_batch, question_batch, ques_len_batch, 1  ) 
                        print 'avn1[0]: ', ['%.4f' % i for i in avn1[0,:].tolist()], '\n'
                        avn2 = model.get_avn(2, img_feat_batch, question_batch, ques_len_batch, 1  )
                        print 'avn2[0]: ', ['%.4f' % i for i in avn2[0,:].tolist()], '\n'
                         
                        aut1 = model.get_aut(1, img_feat_batch, question_batch, ques_len_batch, 1  )
                        print 'aut1[0]: ', ['%.4f' % i for i in aut1[0,:].tolist()], '\n'
                        aut2 = model.get_aut(2, img_feat_batch, question_batch, ques_len_batch, 1  )
                        print 'aut2[0]: ', ['%.4f' % i for i in aut2[0,:].tolist()], '\n'
