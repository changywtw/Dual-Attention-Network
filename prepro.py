...
def build_vocab_question(imgs, params):
    ...
    print 'inserting the special UNK, BLNK token'
    vocab.append('UNK')
    vocab.insert(0, 'BLNK')
...
def encode_mc_answer(imgs, atoi):
    ...
    for i, img in enumerate(imgs):
        for j, ans in enumerate(img['MC_ans']): 
            mc_ans_arrays[i,j] = atoi.get(ans, atoi.get("UNK"))
...
def main(params):
    ...
    atoi = {w:i for i,w in enumerate(top_ans)} # u'yes':0, u'no':1, u'2':2, u'1':3, ...
    itoa = {i:w for i,w in enumerate(top_ans)} # 0: u'yes', 1: u'no', 2: u'2', 3: u'1', ...
    ...
    print "append 'UNK' to answer vocab"
    etc_pos = len(atoi)
    atoi["UNK"]=etc_pos
    itoa[etc_pos]="UNK"
    ...
    # ques_train: 2d array[question_num, question_max_length]: questions' sentence
    f.create_dataset("ques_train", dtype='uint32', data=ques_train)  
    # ques_length_train: 1d array[question_num]: [len of the 1st ques, len of the 2nd ques, ...]
    f.create_dataset("ques_length_train", dtype='uint32', data=ques_length_train) 
    # question_id_train: 1d array[question_num]: [id of the 1st ques, id of the 2nd ques, ...]
    f.create_dataset("question_id_train", dtype='uint32', data=question_id_train) 
    
    # answers: 1d array[question_num]: [answer of 1st quest, ...]
    f.create_dataset("answers", dtype='uint32', data=A)
    # img_pos_train: 1d array, the imgs_train's image_index in unique_img_train 
    ...
    out = {}
    out['ix_to_word'] = itow # encode the (0-indexed) vocab
    out['ix_to_ans'] = itoa
...
if __name__ == "__main__": 
    ...
    # input json
    parser.add_argument('--input_train_json', default='./data/packed_json/vqa_raw_train.json', help='input training json file to process into hdf5')
    parser.add_argument('--input_test_json', default='./data/packed_json/vqa_raw_test.json', help='input testing json file to process into hdf5')
    parser.add_argument('--num_ans', default='1000', type=int, help='number of top answers for the final classifications.')

    
    parser.add_argument('--output_json', default='./data/prepro/data_prepro.json', help='output json file')
    parser.add_argument('--output_h5', default='./data/prepro/data_prepro.h5', help='output h5 file')
